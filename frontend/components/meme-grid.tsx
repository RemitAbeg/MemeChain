    ...
    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
      {memes.map((meme) => (
        <div
          ...
          onClick={() =>
            setSelectedMeme(selectedMeme === meme.id ? null : meme.id)
          }
        >
          {/* Image */}
          <div className="relative aspect-square bg-mc-surface overflow-hidden">
            <div className="absolute inset-0 bg-gradient-to-b from-primary/0 via-transparent to-primary/10 z-10" />
            {meme.imageUrl && meme.imageUrl !== "/placeholder.svg" ? (
              <img
                src={meme.imageUrl}
                alt={`Meme ${meme.id}`}
              ...
              <div className="w-full h-full bg-mc-surface flex items-center justify-center">
                <span className="text-center text-sm text-mc-text/60 p-4">
                  No image
                </span>
              </div>
            )}
          ...
          <div className="absolute inset-0 bg-gradient-to-t from-mc-bg/80 to-transparent opacity-0 group-hover:opacity-100 transition-all flex items-end p-3">
            <div className="w-full space-y-2">
              <div className="flex items-center justify-between text-sm">
                <span className="text-mc-text/80 font-mono text-xs truncate">
                  {meme.creator}
                </span>
